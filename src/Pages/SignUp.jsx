...
const SignUp = () => {
  const { setUser } = useContext(AuthContext); // AuthContext থেকে user update
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [show, setShow] = useState(false);
  ...
  const handleSignUp = (e) => {
    e.preventDefault();
    const displayName = e.target.name?.value || "User";
    const photoURL = e.target.photo?.value || "https://via.placeholder.com/150";
    const emailInput = e.target.email?.value;
    const passwordInput = e.target.password?.value;

    // Password validation: relaxed a bit
    const regExp = /^(?=.*[A-Za-z])(?=.*\d).{6,}$/;
    if (!regExp.test(passwordInput)) {
      toast.error(
        "Invalid password! Must be 6+ chars with letters & numbers."
      );
      return;
    }
  ...
  const handleGoogleSignUp = () => {
    ...
    signInWithPopup(auth, provider)
      .then((res) => {
        toast.success("✅ Google Sign-In Successful!");
        setUser(res.user); // AuthContext update
      })
      .catch((err) => {
        console.error(err);
        toast.error(err.message);
